...
class DetalhesEstatisticaCliente extends StatefulWidget {
  final int? idMes;
  final int? idAno;
  DetalhesEstatisticaCliente({Key? key, @required this.idAno, this.idMes})
      : super(key: key);
  @override
  _DetalhesEstatisticaClienteState createState() =>
      _DetalhesEstatisticaClienteState(idAno: idAno, idMes: idMes);
}

class _DetalhesEstatisticaClienteState
    extends State<DetalhesEstatisticaCliente> {
  _DetalhesEstatisticaClienteState({@required this.idAno, this.idMes}) {
    fetchPost(idAno, idMes);
  }
  int? idMes;
  int? idAno;
  var dadosListagem = <ModelsEstFaturamento>[];
  RxBool? loading = true.obs;

  Future<dynamic> fetchPost(int? idano, int? idmes) async {
    final response = await http.get(
      Uri.parse(EstBuscaDetalhesPorMes +
          idano.toString() +
          '/' +
          idMes.toString() +
          '/' +
    ...
    if (mounted) {
      setState(
        () {
          ...
          dadosListagem = lista
              .map((model) => ModelsEstFaturamento.fromJson(model))
              .toList();
        },
      );
    }
  ...
  Future<dynamic> listarDados() async {
    final response = await http.get(
      Uri.parse(ListaClientesFaturamentoPorMes +
          idAno.toString() +
          '/' +
          idMes.toString() +
          '/' +
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                FieldsDatabase().listaDadosBanco(
                                  'Mês: ',
                                  FieldsEstatisticaFaturamento.nomeMes,
                                  sizeCampoBanco: 22,
                                  sizeTextoCampo: 22,
                                ),
                                SizedBox(height: 15),
                                ...
                                FieldsDatabase().listaDadosBanco(
                                  ...
                                  FieldsEstatisticaFaturamento
                                      .mediaFaturaPedidoMes
                                      .toString(),
                                  sizeCampoBanco: 22,
                                  sizeTextoCampo: 22,
                                ),
                                                        ...
                                                        children: [
                                                          ...
                                                          FieldsDatabase()
                                                              .listaDadosBanco(
                                                                  ...
                                                                  dadosListagem[
                                                                          index]
                                                                      .faturamentoClienteMes),
                                                          SizedBox(
                                                            height: 5,
                                                          ),
                                                          FieldsDatabase()
                                                              .listaDadosBanco(
                                                            'Representação faturamento mês: ',
                                                            dadosListagem[index]
                                                                    .percentualFaturaClienteMes
                                                                    .toString() +
                                                                ' %',
                                                          )
                                                        ],
